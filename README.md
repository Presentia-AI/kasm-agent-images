...
## ClickUp DM bridge: `BRIDGE_ENABLED`

`__presentia_ensure_dm_bridge` starts the ClickUp DM bridge from the tooling repo
(`bridge/bridge-ctl`), which lets Claude's voice mode — or any allowlisted ClickUp
...
```json
{"environment": {"AGENT_ROLE": "dev", "BRIDGE_ENABLED": "1", "CLICKUP_AGENT_TOKEN": "pk_..."}}
```

`bridge.env` is generated by the hook rather than mounted as a new host secret —
the only secret it needs is `CLICKUP_AGENT_TOKEN`, which run_config already
injects. It is written `600`, and an existing file is never overwritten. Every
other setting has a default that run_config can override:

| Variable | Default |
|---|---|
| `BRIDGE_CHANNEL_ID` | `8cnhc1c-2875` |
| `BRIDGE_ALLOWED_SENDERS` | `74542365` |
| `BRIDGE_SELF_USER_ID` | `106678273` |
| `BRIDGE_REQUIRED_PREFIX` | `@claude,at claude,at-claude,hey claude` |
| `BRIDGE_REPLY_STYLE` | `voice` |
| `BRIDGE_POLL_SECONDS` | `5` |
| `BRIDGE_PERMISSION_MODE` | `bypassPermissions` |

If `BRIDGE_ENABLED=1` but the token is unset or the tooling clone predates the
bridge, the hook warns on stderr and continues — it never blocks the boot.
